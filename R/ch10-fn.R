...
ch10.man = function(fn=0) {
    if (0 %in% fn) {
	cat("[1] pmean.ci\t\tConfidence Interval for a Mean (known variance)\n")
	cat("[2] meantest1.plot \tHypothesis Test for a Mean (known variance)\n")
	cat("[3] pmean.ci2\t\tConfidence Interval for the Mean (unknown variance)\n")
	cat("[4] meantest2.plot \tHypothesis Test for a Mean (unknown variance)\n")
	cat("[5] prob.ci\t\tConfidence Interval for a Proportion\n")
	cat("[6] bntest.plot\t\tPlot of Exact Binomial Test\n")
	cat("[7] bntest2.plot  \tPlot of Three Types of Binomial Tests\n")
	cat("[8] var1.ci\t\tConfidence Interval for a Variance\n")
	cat("[9] var1.test\t\tHypothesis Test for a Variance\n")
	cat("[10] chitest.plot   \tPlot of the Chi-square Test\n")
	cat("[11] cimean.sim    \tSimulate the Confidence Interval for a Mean\n")
	cat("[12] civar.sim\t\tSimulate the Confidence Interval for a Variance\n")
	cat("[13] ciprob.sim\t\tSimulate the Confidence Interval for a Proportion\n")
	cat("[14] meanpower.plot \tPower Function of the Test for a Mean\n")
	cat("[15] meanchar.plot  \tOperating Characteristic Curves of the Test for a Mean\n")
    }
    if (1 %in% fn) {
    ...
    if (6 %in% fn) {
	cat("[6] Plot of Exact Binomial Test\n")
    ...
    if (7 %in% fn) {
	cat("[7] Plot of Three Types of Binomial Tests\n")
    ...
    if (10 %in% fn) {
	cat("[10] Plot of the Chi-square Test\n")
...
#' meantest1.plot(xb=12.64, mu0=12.5, sig=0.5, n=40, side="two")
#'
#' set.seed(4989)
#' x = rnorm(40, 12.64, 0.5)
#' meantest1.plot(x, mu0=12.5, sig=0.5, side="two")
...
#' meantest2.plot(xb=12.65, mu0=12.5, sig=0.57, n=40, side="two")
#'
#' set.seed(1234)
#' x = rnorm(40, 12.65, 0.57)
#' meantest2.plot(x, mu0=12.5, side="two")
...
#' @examples
#' prob.ci(n=200, x=15)
#' prob.ci(n=200, x=11:15)
#' prob.ci(n=200, x=15, alp=c(0.1, 0.05, 0.01))
#' @rdname prob.ci
#' @export
prob.ci = function(n, x, alp=0.05, dig=4) {
	...
	p = x/n
	if (length(alp)>1) p = rep(p, length(alp))
	err = qnorm(1-alp/2)*sqrt(p*(1-p)/n)
     # [Correction]
	nn = max(length(x), length(alp))
	for (k in 1:nn) {
	    cat(paste0("[", round(p[k], dig), " \U00B1 ", round(qnorm(1-alp[k]/2), dig), "\U00D7\U221A(",
		round(p[k], dig), "\U00D7", round(1-p[k], dig), "/", n,
		")] = [", round(p[k], dig), " \U00B1 ", round(err[k], dig), "] = [",
		round(p[k]-err[k], dig), ", ", round(p[k]+err[k], dig),"]"), "\n")
	}
}
...
#'
#' ciprob.sim(n=100, p=0.6, alp=0.05, N=10000, plot=FALSE)
#' @rdname ciprob.sim
#' @export
...
#' @examples
#' meanpower.plot()
#' meanpower.plot(mu1=0.3)
#'
#' meanpower.plot(mu0=100, mu1=102, sig=5, side="up")
#' n = c(10, 30, 50, 70, 100)
#' meanpower.plot(mu0=100, mu1=102, sig=5, nv=n)
#'
#' meanpower.plot(nv=50:150)
...
meanpower.plot = function(mu0, mu1, sig, nv, alp=0.05, prng, side="two", mt, dig=4) {
	...
	if (missing(prng)) prng = switch(nside, c(mu0-4*se0, mu0), c(mu0, mu0+4*se0), c(mu0-4*se0, mu0+4*se0))
        # [Correction]
	if (!(missing(mu1))) { x1 = prng[1]-(prng[2]-prng[1])*0.15
	} else {x1 = prng[1]}
...
#' @examples
#' meanchar.plot()
#' meanchar.plot(mu1=0.3)
#'
#' meanchar.plot(mu0=100, mu1=98, sig=5, side="low")
#' n = c(10, 30, 50, 70, 100)
#' meanchar.plot(mu1=0.3, nv=n)
#' meanchar.plot(mu0=100, mu1=102, sig=5, nv=n)
#'
#' meanchar.plot(nv=50:150)
