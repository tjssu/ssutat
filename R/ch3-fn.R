...
ch3.man = function(fn=0) {
	...
	cat("[2] pprt    \t\tCalculate Probability of an Event\n")
	cat("[3] cprt    \t\tCalculate the Conditional Probability\n")
	cat("[4] indep.event\t\tDetermine Independence of Two Discrete Random Variables\n")
	cat("[5] bayes.plot\t\tDisplay the Prior and the Posterior Probabilities\n\n")
	
	cat("Functions of the \"prob\" package -----------------\n")
	cat("[P1] rolldie\t\tCreate Sample Space of Rolling Dice\n")
	cat("[P2] tosscoin\t\tCreate Sample Space of Tossing Coins\n")
	cat("[P3] cards  \t\tCreate Sample Space of Sampling Cards\n")
	cat("[P4] urnsamples\t\tCreate Sample Space of Urn Sampling\n")
	cat("[P5] union \t\tUnion of Events in Data frame Format\n")
	cat("[P6] intersect\t\tIntersection of Events in Data frame Format\n")
	cat("[P7] setdiff \t\tSet Difference of Events in Data frame Format\n\n")
    }
    if (1 %in% fn) {
    ...
    if (2 %in% fn) {
	cat("[2] Calculate the Probability of an Event\n")
    ...
    if (3 %in% fn) {
	cat("[3] Calculate the Conditional Probability\n")
	cat("cprt(a, b)\n")
	cat("[Mandatory Input]--------------------------\n")
    ...
    if (4 %in% fn) {
	cat("[4] Determine Independence of Two Discrete Random Variables\n")
    ...
    if (5 %in% fn) {
	cat("[5] Display the Prior and the Posterior Probabilities\n")
...
pprt = function(x, n, prt=TRUE) { 
	...
	# [corr]
	numer = ifelse (is.null(x), 0, nrow(x))
	if (prt==TRUE) cat(paste0("P(", en, ") ="), numer, "/", n, "=", numer/n,"\n")
	invisible(numer/n)
}
...
pprt2 = function(x, xn, n, prt=TRUE) {
	# [corr]
	if (missing(xn)) xn = deparse(substitute(x))
	numer = ifelse(is.null(x),0, nrow(x))
	if (prt==TRUE) cat(paste0("P(", xn, ") ="), numer, "/", n, 
		"=", numer/n,"\n")
	invisible(numer/n)
}
...
#' B = subset(S, (X1+X2) >=8)
#' cprt(B, A)
#' cprt2(B, "Sum >=8", A, "At least one 6")
#' @rdname cprt
#' @export 
cprt = function(a, b) {
	# [corr]
	...
	numer = ifelse (is.null(ab), 0, nrow(ab))
    	cat(paste0("P(",an,"|",bn,")="), numer,"/",nrow(b),
		"=", numer/nrow(b),"\n")
}
...
#' @export 
cprt3 = function(a, N1, b, N2) {
	if (missing(N1)) stop("Input the size of the first sample sapce!")
	if (missing(N2)) stop("Input the size of the second sample sapce!")
	# [corr]
	if (is.null(b)) stop("The conditioning event must not be empty!")
	ab = intersect2(a, b)
	# [corr]  
	numer = ifelse (is.null(ab), 0, nrow(ab))
	NN = max(N1, N2)
    	cat(paste0("P(",an,"|",bn,") = ("), numer,"/",NN, "(",nrow(b),"/",N2,
		") =", numer/NN/(nrow(b)/N2))
}
...
#' @examples 
#' S=prob::rolldie(3); (N=nrow(S))
#' even=function(x) (sum(x)%%2)==0
#' span5=function(x) (max(x)-min(x))==5
#' A = prob::subset(S, apply(S,1,even)); nrow(A)
#' B = prob::subset(S, apply(S,1,span5)); nrow(B)
