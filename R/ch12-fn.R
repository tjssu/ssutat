...
ch12.man = function(fn=0) {
	...
	cat("[3] cross.test     \tTest of Homogeneity / Independence\n")
	cat("[4] mosaic2\t\tMosaic Plot with Pearson Residuals\n")
    }
    if (1 %in% fn) {
	cat("[1] Goodness of Fit Test from a Table\n")
	cat("chi.gof1(x, p, alp=0.05, plot=TRUE, dig=2, dig2=4)\n")
	cat("[Mandatory Input]--------------------------\n")
	cat("x\t Table of frequency\n")
	cat("[Optional Input]--------------------------\n")
	cat("p\t Probability vector\n")
	cat("alp\t Level of significance (default=0.05)\n")
	cat("plot\t Logical value for plotting the test result (default=TRUE)\n")
	...
	cat("chi.gof2(tab, para, para2, dist=\"pois\", mc1, mc2, alp=0.05, plot=TRUE, dig=2, dig2=4)\n")
	cat("[Mandatory Input]--------------------------\n")
    ...
    if (3 %in% fn) {
	cat("[3] Test of Homogeneity / Independence\n")
	cat("cross.test(x, v1, v2, alp=0.05, prt=TRUE, plot=TRUE, dig=2, dig2=4)\n")
	cat("[Mandatory Input]--------------------------\n")
    ...
    if (4 %in% fn) {
	cat("[4] Mosaic Plot with Pearson Residuals\n")
	cat("require(vcd)\n")
...
#' @param plot Logical value for plotting the test result, Default: TRUE
...
#' @rdname chi.gof1
#' @export
chi.gof1 = function(x, p, alp=0.05, plot=TRUE, dig=2, dig2=4) {
	...
	cat("alpha =", alp, "\tP-value =", format(pv, F, dig2), "\n")
       # Plot the Test Results
	if (plot) chitest.plot2(stat, df=k-1, alp=alp, side="up", pup=max(0.999, 1-pv/2))
...
#' @rdname chi.gof2
#' @export
chi.gof2 = function(tab, para, para2, dist="pois", mc1, mc2, alp=0.05, plot=TRUE, dig=2, dig2=4) {
      # Frequency Table
	...
	cat("alpha =", alp, "\tP-value =", format(pv, F, dig2), "\n")
       ...
       # Plot the Test Results
	if (plot) chitest.plot2(stat, df=df, alp=alp, side="up", pup=max(0.999, 1-pv/2))

	invisible(list(stat=stat, df=df, crv=crv, pv=pv, tab=tab1))
}

# [12-3] Test of Homogeneity / Independence from a Cross Table
#' @title Test of a Cross Table
#' @description Test of Homogeneity / Independence from a Cross Table
#' @param x Cross Table
...
#' @rdname cross.test
#' @export
cross.test = function(x, v1, v2, alp=0.05, prt=TRUE, plot=TRUE, dig=2, dig2=4) {
	...
	if (stat > crv) {sign = ">"
		...
		ans = "Accept H0"}
	cat("GoF Stat =", round(stat, dig2), sign, round(crv, dig2), "\U21D2", ans, "\n")
	cat("Df =", df, "alpha =", alp, "\tP-value =", format(pv, digits=dig2), "\n")
...
# [12-4] Mosaic Plot with Pearson Residuals
#' @title Mosaic Plot with Pearson Residuals
#' @description Mosaic Plot Displaying Pearson Residuals.
...
mosaic2 = function(tab, mt, dig=4, resid=TRUE) {
        # [Correction]
	if (missing(mt)) mt = paste("Mosaic Plot of", deparse(substitute(tab)))

	vcd::mosaic(tab, shade=T, pop=F, main=mt,
        		labeling_args=list(offset_varnames=c(top=1), offset_labels=c(top=0.3)))
        # [Correction]
	if (length(dim(tab))==2) {
	  ct = chisq.test(tab)
	  cat("Test statistic =", round(ct$stat, dig), "\t df =", ct$para,
		"\t P-value =", format(ct$p.val, F, dig), "\n")
...
#' @export
chitest.plot2 = function(stat, df, alp=0.05, side="two", pup=0.999, dig=4, ppt=50) {
	...
	if (side=="up" | grepl(side, "greater")) {
		...
		text(xpv, ypv, round(pv, dig), pos=pospv, col="red")
	    # text(stat, 0, round(stat, dig), pos=1, col="blue")
		segments(rej, 0, rej, rejy, lwd=1, col="red")
		ry0 = ifelse (abs(rej-stat)<2, rejy, 0)
	    # text(rej, ry0, labels=round(rej, dig), pos=1, col="blue")
		...
		if (pv>0.7) postat=3
		text(stat, staty, labels=bquote(chi[0]^2 == .(round(stat, dig))),
			pos=postat, col="blue", cex=1)
		text(rej, rejy, labels=bquote(chi[.(paste(1-alp,";",df))]^2 == .(round(rej, dig))),
			pos=3, col="red", cex=1)
		...
		text(stat, dchisq(stat, df), round(pv, dig), pos=2, col="red")
		text(stat, 0, round(stat, dig), pos=1, col="blue")
		...
		text(c(mlow, mup), dchisq(c(mlow, mup), df), round(pv/2, dig), pos=c(2,4), col="red")
