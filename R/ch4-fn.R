...
ch4.man = function(fn=0) {
	...
	cat("[14] cont.trans\t  Transformed PDF of a Continuous Random Variable\n")
    }
    if (1 %in% fn) {
    ...
    if (12 %in% fn) {
	cat("[12] Independence of Two Discrete Random Variables\n")
    ...
    if (13 %in% fn) {
	cat("[13] Independence of Two Continuous Random Variables\n")
...
#' Tail_10=0:10
#' disc.cdf(Tail_10, choose(10,Tail_10), cex=0.7)
#'
#' B10.7=0:10
#' disc.cdf(B10.7, dbinom(B10.7, 10, 0.7), cex=0.8)
#' @rdname disc.cdf
#' @export
...
dbl.int = function(FUN, x1, x2, y1, y2) {
...
#' cont.jcdf(pdf, 0:5/5, 0:5/5)
#' @rdname cont.jcdf
#' @export
cont.jcdf = function(FUN, xs, ys, lo1=-Inf, lo2=-Inf) {
    # Define CDF
	Fxy = function(x, y) dbl.int(FUN, lo1, x, lo2, y)[[1]]
    # Vectorize CDF
...
cont.jcdfp = function(FUN, lo1, up1, lo2, up2, mt) {
    ...
    # Plot CDF F(x, y)
	  ...
	  # Display graph
	  win.graph(7, 5)
	  s3d = scatterplot3d::scatterplot3d(xa, ya, VFxy(xa, ya), highlight.3d=TRUE,
...
#' @return None.
#' @examples
#' x = function(x) dnorm(x, 10, 2)
#' ty = function(x) 10*x - 4
#' tw = function(x) -10*x + 4
#' fy = function(y) fx((y+4)/10)/10
#' fw = function(w) fx((-w+4)/10)/10
#' cont.trans(fx, list(y=ty, w=tw), list(fy, fw), a=9, b=12, lo=6, up=14, plot=TRUE, cex=1.3)
#' @rdname cont.trans
#' @export
