...
ch8.man = function(fn=0) {
	...
	cat("[2] norm.trans\t\tCheck Probability Conservation in Standardization\n")
    ...
    if (2 %in% fn) {
	cat("[2] Check Probability Conservation in Standardization\n")
	cat("norm.trans(mu, sig, a, b, mt1, mt0, dig=4, span=6, np=100)\n")
	cat("[Mandatory Input]--------------------------\n")
	...
	cat("span\t Range of x-axis (mu \U00B1 span) (default=6)\n")
...
#' cont.spdf("norm", -5, 10, mu, sig, xp=mu)
#' @rdname cont.spdf
#' @export
...
#' @param span Range of x-axis in [mu-span, mu+span], Default: 6
...
#' @rdname norm.trans
#' @export
norm.trans = function(mu, sig, a, b, mt1, mt0, dig=4, span=6, np=100) {
	...
	if (sig >=3 | sig <=1/3) x1 = seq(mu - span*sig, mu + span*sig, length=np)
	...
	par(mar=c(3,4,3,1))
    # Plot the normal PDF
	plot(x1, fx[,2], type="n", main=mt1, ylim=c(0, ymax*ysc1), ylab="f(x)", xlab="")
...
# [8-5 old] Quantile Plot of the Standard Normal Distribution
#' @title Quantile Plot of the Standard Normal Distribution
...
# [8-5 rev.] Quantile Plot of the Standard Normal Distribution
#' @title Quantile Plot of the Standard Normal Distribution
...
#' @examples
#' pv = c(0.005, 0.01, 0.025, 0.05, 1:9/10, 0.95, 0.975, 0.99, 0.995)
#' pv2 = c(0.005, 0.025, 0.05, 0.95, 0.975, 0.995)
#' snorm.quant2(pv, pv2)
#' @rdname snorm.quant
#' @export
snorm.quant2 = function(pv, pv2, mt, dig=4) {
	if (missing(mt)) mt = "Quantiles of the Standard Normal Distribution"
      # Quantiles
	zv = qnorm(pv)
	names(zv) = pv
