...
ch11.man = function(fn=0) {
    if (0 %in% fn) {
	cat("[1] mean2.ci\t\tConfidence Interval for the Difference of Two Means\n")
	cat("[2] mean2test.plot \tHypothesis Test for the Difference of Two Means\n")
	cat("[3] normtest.plot  \tPDF Plot of the Z-Test Statistic\n")
	cat("[4] ttest.plot\t\tPDF Plot of the t-Test Statistic\n")
	cat("[5] prob2.ci\t\tConfidence Interval for the Difference of Proportions\n")
	cat("[6] prob2test.plot  \tTest for the Difference of Proportions (Large Sample)\n")
	cat("[7] ftest.plot\t\tPDF Plot of the F-Test Statistic\n")
	cat("[8] var2.ci\t\tConfidence Interval for the Ratio of Two Variances\n")
	cat("[9] var2.test\t\tHypothesis Test on Two Variances\n")
	cat("[10] civar2.sim\t\tSimulate the Confidence Interval for Variance Ratios\n")
    }
    if (1 %in% fn) {
    ...
    if (3 %in% fn) {
	cat("[3] PDF Plot of the Z-Test Statistic\n")
    ...
    if (4 %in% fn) {
	cat("[4] PDF Plot of the t-Test Statistic\n")
    ...
    if (7 %in% fn) {
	cat("[7] PDF Plot of the F-Test Statistic\n")
	...
	cat("n2\t Sample size of population2\n")
	cat("[Optional Input]--------------------------\n")
	cat("sig1\t Standard deviation of population1 (default=1)\n")
	cat("sig2\t Standard deviation of population2 (default=1)\n")
	cat("alp\t Level of significance (default=0.05)\n")
...
mean2test.plot = function(xb1, xb2, s1, s2, n1, n2, d0=0, prng, side="two", pvar="equal", mt, dig=4, xlab) {
	...
	if (side=="up" | grepl(side, "greater")) {
		...
		segments(xd, 0, xd, dnorm(xd, d0, se), lwd=2, col="red")
		text(xd, dnorm(xd, d0, se)*0.9, round(pv, 4), pos=4, col="red")
		text(xd, 0, round(xd, 4), pos=1, col="blue")
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(xd, 0, xd, dnorm(xd, d0, se), lwd=2, col="red")
		text(xd, dnorm(xd, d0, se)*0.9, round(pv, 4), pos=2, col="red")
		text(xd, 0, round(xd, 4), pos=1, col="blue")
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(c(mlow, mup), 0, c(mlow, mup), dnorm(xd, d0, se), lwd=2, col="red")
		text(c(mlow, mup), dnorm(xd, d0, se)*0.9, round(pv/2, 4), pos=c(2,4), col="red")
		text(c(mlow, mup), 0, round(c(mlow, mup), 4), pos=1, col="blue")
	}
	abline(h=0); abline(v=d0, lty=2, lwd=2, col="green3")
	lines(xa, dnorm(xa, d0, se), type="l", lwd=2, col="blue")
...
normtest.plot = function(md, mu0=0, se=1, prng=c(-4,4), side="two", xlab="Sample Mean", pvout=TRUE) {
	...
	if (side=="up" | grepl(side, "greater")) {
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(md, 0, md, dnorm(md, mu0, se), lwd=2, col="red")
		text(md, dnorm(md, mu0, se)*0.9, round(pv, 4), pos=4, col="red")
		text(md, 0, round(md, 4), pos=1, col="blue")
	} else if (side=="low" | grepl(side, "less")) {
		pv = pnorm(md, mu0, se)
		if (pvout) cat("P-v =", pv, "\n")
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(md, 0, md, dnorm(md, mu0, se), lwd=2, col="red")
		text(md, dnorm(md, mu0, se)*0.9, round(pv, 4), pos=2, col="red")
		text(md, 0, round(md, 4), pos=1, col="blue")
	} else if (grepl(side, "two sided")) {
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(c(mlow, mup), 0, c(mlow, mup), dnorm(md, mu0, se), lwd=2, col="red")
		text(c(mlow, mup), dnorm(md, mu0, se)*0.9, round(pv/2, 4), pos=c(2,4), col="red")
		text(c(mlow, mup), 0, round(c(mlow, mup), 4), pos=1, col="blue")
	}
...
ttest.plot = function(md, deg, prng=c(-4,4), side="two", dig=4, mt, pvout=TRUE) {
	...
	if (side=="up"| grepl(side, "greater")) {
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(md, 0, md, dt(md, deg), lwd=2, col="red")
		text(md, dt(md, deg)*0.9, round(pv, dig), pos=4, col="red")
		text(md, 0, round(md, dig), pos=1, col="blue")
	} else if (side=="low"| grepl(side, "less")) {
		pv = pt(md, deg)
		if (pvout) cat("P-v =", pv, "\n")
		cord.x = c(prng[1], seq(prng[1], md, length.out=20), md)
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(md, 0, md, dt(md, deg), lwd=2, col="red")
		text(md, dt(md, deg)*0.9, round(pv, dig), pos=2, col="red")
		text(md, 0, round(md, dig), pos=1, col="blue")
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(c(mlow, mup), 0, c(mlow, mup), dt(md, deg), lwd=2, col="red")
		text(c(mlow, mup), dt(md, deg)*0.9, round(pv/2, dig), pos=c(2,4), col="red")
		text(c(mlow, mup), 0, round(c(mlow, mup), dig), pos=1, col="blue")
	}
	abline(h=0); abline(v=0, lty=2, lwd=2, col="green3")
	lines(xa, dt(xa, deg), type="l", lwd=2, col="blue")
}
...
#'
#' @examples
#' prob2.ci(n1=160, x1=12, n2=200, x2=13)
#' prob2.ci(n1=160, x1=12, n2=200, x2=13, alp=0.1)
#' prob2.ci(n1=160, x1=12, n2=200, x2=13, dig=3)
#' @rdname prob2.ci
#' @export
...
#'
#' @examples
#' prob2test.plot(n1=150, x1=12, n2=250, x2=10)
#' prob2test.plot(n1=150, x1=12, n2=250, x2=10, side="up")
...
prob2test.plot = function(n1, x1, n2, x2, prng, side="two", mt, dig=4) {
	...
	if (side=="up" | grepl(side, "greater")) {
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(xd, 0, xd, dnorm(xd, 0, se), lwd=2, col="red")
		text(xd, dnorm(xd, 0, se)*0.9, round(pv, 4), pos=4, col="red")
		text(xd, 0, round(xd, 4), pos=1, col="blue")
	} else if (side=="low" | grepl(side, "less")) {
		pv = pnorm(z0)
		cat("P-v = P(Z < Z0) =", round(pv, dig), "\n")
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(xd, 0, xd, dnorm(xd, 0, se), lwd=2, col="red")
		text(xd, dnorm(xd, 0, se)*0.9, round(pv, 4), pos=2, col="red")
		text(xd, 0, round(xd, 4), pos=1, col="blue")
	} else if (grepl(side, "two sided")) {
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(c(mlow, mup), 0, c(mlow, mup), dnorm(xd, 0, se), lwd=2, col="red")
		text(c(mlow, mup), dnorm(xd, 0, se)*0.9, round(pv/2, 4), pos=c(2,4), col="red")
		text(c(mlow, mup), 0, round(c(mlow, mup), 4), pos=1, col="blue")
	}
	abline(h=0); abline(v=0, lty=2, lwd=2, col="green3")
	lines(xa, dnorm(xa, 0, se), type="l", lwd=2, col="blue")
}
...
#' (vo = var.test(x, y))
#' win.graph(7, 5)
#' ftest.plot(fstat=vo$stat, deg=as.vector(vo$para), pmax=0.999, side="two")
#' @rdname ftest.plot
#' @export
ftest.plot = function(fstat, deg, pmax=0.995, side="two") {
	...
	if (side=="up" | grepl(side, "greater")) {
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(fstat, 0, fstat, df(fstat, deg[1], deg[2]), lwd=2, col="red")
		text(fstat, df(fstat, deg[1], deg[2])*0.9, round(pv, 4), pos=4, col="red")
		text(fstat, 0, round(fstat, 4), pos=1, col="blue")
	} else if (side=="low" | grepl(side, "less")) {
		pv = plow
		cat("P-v =", pv, "\n")
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(fstat, 0, fstat, df(fstat, deg[1], deg[2]), lwd=2, col="red")
		text(fstat, df(fstat, deg[1], deg[2])*0.9, round(pv, 4), pos=2, col="red")
		text(fstat, 0, round(fstat, 4), pos=1, col="blue")
	} else if (grepl(side, "two sided")) {
		...
		polygon(cord.x, cord.y, col='lightcyan')
		segments(c(mlow, mup), 0, c(mlow, mup), df(c(mlow), deg[1], deg[2]), lwd=2, col="red")
		text(c(mlow, mup), df(c(mlow), deg[1], deg[2]), round(pv/2, 4), pos=c(2,4), col="red")
		text(c(mlow, mup), 0, round(c(mlow, mup), 4), pos=1, col="blue")
	}
	abline(h=0); abline(v=qf(0.5, deg[1], deg[2]), lty=2, lwd=2, col="green3")
	lines(xa, df(xa, deg[1], deg[2]), type="l", lwd=2, col="blue")
}
...
# [11-10] Simulate the Confidence Interval for a the Ratio of Population Variances
...
civar2.sim=function(n1, n2, sig1, sig2, alp=0.05, N=100, seed=9857, dig=4, plot=TRUE) {
	...
	if (missing(sig1)) {sig1 = 1
		cat("Since sig1 is missing, it is automatically set to 1.\n")}
	if (missing(sig2)) {sig2 = 1
		cat("Since sig2 is missing, it is automatically set to 1.\n")}
	vr0 = sig1^2/sig2^2
