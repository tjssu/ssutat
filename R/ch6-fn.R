...
ch6.man = function(fn=0) {
	...
	cat("[2] multinorm.plot \tDisplay of Multinomial PDF\n")
    }
    if (1 %in% fn) {
    ...
    if (2 %in% fn) {
	cat("[2] Display of Multinomial PDF\n")
...
multinorm.plot = function(ps, size) {
	...
	for (k in 1:ng) scatterplot3d::scatterplot3d(x[[1]], x[[2]], fx6[,k], type="h", main=mt6[k],
