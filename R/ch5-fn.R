...
ch5.man = function(fn=0) {
	...
	cat("[2] disc.jexp\tExpected Values of Two Discrete Random Variables\n")
	cat("[3] cont.jexp\tExpected Values of Two Continuous Random Variables\n")
	cat("[4] corr.plot\tCorrelation Coefficients and Scatter Plots\n")
    }
    if (1 %in% fn) {
    ...
    if (2 %in% fn) {
	cat("[2] Expected Values of Two Discrete Random Variables\n")
    ...
    if (3 %in% fn) {
	cat("[3] Expected Values of Two Continuous Random Variables\n")
    ...
    if (4 %in% fn) {
	cat("[4] Correlation Coefficients and Scatter Plots\n")
...
#'
#' x = (0:10)^2
#' disc.exp(x, dbinom(0:10, 10, 0.5), plot=TRUE)
#' @rdname disc.exp
#' @export
...
cont.jexp = function(FUN, lo1=-Inf, up1=Inf, lo2=-Inf, up2=Inf, dig=4, prt="exp") {
	...
	Ey = Exp(ey1, lo1, up1, lo2, up2)[[1]]
	Ey2 = Exp(ey2, lo1, up1, lo2, up2)[[1]]
	Vy = Ey2 - Ey^2
	Dy = sqrt(Vy)
    # Covariance and correlation of random variable X and Y
	exy = function(x, y) x*y*FUN(x, y)
	Exy = Exp(exy, lo1, up1, lo2, up2)[[1]]
	Vxy = Exy - Ex*Ey
	Cxy = Vxy/sqrt(Vx*Vy)
    # Added (2020.05.19)
            if (prt == c("prob")) {
                prb = Exp(FUN, lo1, up1, lo2, up2)[[1]]
