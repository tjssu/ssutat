...
ch7.man = function(fn=0) {
    ...
    if (2 %in% fn) {
	cat("[2] PDF and CDF Plots for Continuous Random Variables\n")
...
#' cont.exp(fx, 0, 40, mt="X ~ Gam(3,5)", prt=T, plot=T)
#' @rdname cont.exp
#' @export
...
#'
#' alp=10:100/10; th =1
#' cont.mpdf("gamma", 0, 20, para=alp, para2=th)
#' cont.mpdf("weibull", 0, 5, para=alp, para2=th)
#' @rdname cont.mpdf
#' @export
