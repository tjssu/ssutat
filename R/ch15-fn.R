...
ch15.man = function(fn=0) {
	...
	cat("[4] corr.spear\t\tPearson & Spearman Correlation Coefficient\n")
	...
	cat("\t 1\t Pearson correlation coefficient, test and the CI\n")
	cat("\t 2\t Spearman correlation coefficient, test and the CI\n")
	cat("\t 3\t Scatter plot\n")
	cat("alp\t Level of significance (default=0.05)\n")
	cat("dig\t Number of digits below the decimal point (default=4)\n")
    }
	...
	cat("plot\t Logical value for plotting the distribution (default=FALSE)\n")
	cat("dig\t Number of digits below the decimal point (default=4)\n\n")
    }
    if (any(c(6, 62) %in% fn)) {
...
cvruns.exact = function (alpha=0.05, n1, n2) {
	...
	pv1 = randomizeBE::pruns.exact(2, n1, n2)
	if (pv1 > alpha) {
		cr1 = NA
	} else {
		for (r in 2:nmin) 	{
			cp = randomizeBE::pruns.exact(r, n1, n2)
			if (cp <= alpha) cr1 = r 	}
	}
    # Upper critical value
	pv2 = randomizeBE::pruns.exact(rmax, n1, n2)
	if (pv2 > alpha) {
		cr2 = NA
	} else {
		for (r in rmax:nmax) 	{
...
{	if (r <= 1) stop("Number of runs must be >1")
	pv1 = ifelse(r<=2, 0, randomizeBE::pruns.exact(r-1, n1, n2, tail="lower"))
	randomizeBE::pruns.exact(r, n1, n2, tail="lower") - pv1
}
...
runstest.plot = function(x, n1, n2, alp=0.05, side="two", dig=4, plot=TRUE) {
	...
	if (any(grepl(side, c("low", "less")))) {
		pv = randomizeBE::pruns.exact(r1, n1, n2, tail="lower")
		...
		pv = randomizeBE::pruns.exact(r1, n1, n2, tail="upper")
		...
		pv = randomizeBE::pruns.exact(r1, n1, n2, tail="2-sided")
		h1 = "Two-sided"
		r2 = mu + (mu-r1)
		...
		pv2 = 2*pnorm(-abs(z0))
		plow = randomizeBE::pruns.exact(min(r1,r2), n1, n2, tail="lower")
		pupp = randomizeBE::pruns.exact(max(r1,r2), n1, n2, tail="upper")
		ppv = c(plow, pupp)
	}
    # Print test results -------------------------------
...
#' signrank.plot(x, y, side="up")
#' signrank.plot(y, x, side="low")
...
kruswall.plot = function(x, y, dig=4, plot=FALSE) {
	...
	pv = pchisq(H, kk-1, lower.tail=FALSE)
...
friedman.plot = function(x, a, b, dig=4, plot=FALSE) {
	...
	cat(paste0("F = (12 / ", kk," / ",kk+1," / ",rr, ") \U00D7 ", sum(ra^2),
		" - 3 \U00D7 ",rr," \U00D7 ",kk+1, " = ", round(F0, dig)), "\n")
	cat(paste0("df=", kk-1, "\t p-value=", round(pv, dig)), "\n")

    # Plot distribution of test statistic
	if (plot) {
		# Utilize chitest.plot2( ) function in chapter 12
		chitest.plot2(stat=F0, df=kk-1, side="up")
	}
}
